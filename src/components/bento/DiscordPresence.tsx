...
const DiscordPresence = () => {
  ...
  const mainActivity = useMemo(() => {
    ...
    return lanyard.data.activities.find((activity) => activity.type === 0 && activity.assets)
  ...
  useEffect(() => {
    ...
    const updateElapsedTime = () => {
      if (mainActivity.timestamps?.start) {
        setElapsedTime(getElapsedTime(mainActivity.timestamps.start.toString()))
      }
    }
            ...
            {hasMainActivity && mainActivity && mainActivity.assets ? (
              <div className='flex w-full items-center gap-x-3'>
                ...
                <div className='my-2 flex min-w-0 flex-1 flex-col gap-y-1 overflow-hidden'>
                  {mainActivity.name && (
                    <div className='mb-0.5 truncate text-xs leading-none'>{mainActivity.name}</div>
                  )}
