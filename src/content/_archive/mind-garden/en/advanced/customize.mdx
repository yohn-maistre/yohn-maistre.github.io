...
import { Aside, Spoiler, Steps } from '@yohn-maistre/astro-pure-fork/user'
...
6. Use as component library for other Astro projects (yes, this project can be broken down into the UI component library): [Package details](https://www.npmjs.com/package/@yohn-maistre/astro-pure-fork).

Before you customize it, you should check [`site.config.ts`](/mind-garden/setup/configuration#theme-configuration) to make sure there is no option that satisfy your need.
...
4. Solve the sub-dependencies problem:
   ...
   import { Icon } from '@yohn-maistre/astro-pure-fork/user' // [!code ++]
   // ...
   ---
   ```
...
5. Change the reference in `BaseLayout.astro`:
   ...
   ```astro title="src/layout/BaseLayout.astro"
    ---
    import { Footer, Header, ThemeProvider } from '@yohn-maistre/astro-pure-fork/components/basic' // [!code --]
    import { Header, ThemeProvider } from '@yohn-maistre/astro-pure-fork/components/basic' // [!code ++]
...
<Aside type='tip'>
For bun user, please refer to [Deployment#Package Mode](/mind-garden/setup/deployment#package-mode)
