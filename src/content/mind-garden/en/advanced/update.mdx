...
- Add the env variable `BUN_LINK_PKG`. Set this to `true` to automatically refer to the package `@yohn-maistre/astro-pure-fork` in the path `packages/pure` instead of `node_modules/@yohn-maistre/astro-pure-fork`. This may help users develop the theme or change code for themselves (not recommended for production).
...
- This changes import a new component `Svg`. See [User Components #Svg Loader](/mind-garden/integrations/components#svg-loader) for usage.

### v4.0.0-alpha

- Move all components, types and utils that are reused to new npm packages `@yohn-maistre/astro-pure-fork`. Affected components are included in . Import method is also changed:

  ```ts
  import { Button } from '@/components/user' // [!code --]
  import { Button } from '@yohn-maistre/astro-pure-fork/user' // [!code ++]
  ```
...
- New format config file `src/site.config.ts`. There will be zod schema to validate the config file, and use of config will be changed to:
  ...
  import config from 'virtual:pure-config' // [!code ++]
  ```

- Built-in astro config supported. package `@yohn-maistre/astro-pure-fork` will automatically config some integrations like `unocss/astro`, `@astrojs/mdx`, `@astrojs/sitemap`, also with some rehypePlugins and remarkPlugins.
...
- Move advanced components to `src/components/advanced`. Affected components are included in [Advanced Components](/mind-garden/integrations/advanced). Import method is also changed:

  ```ts
  import LinkPreview from '@/components/LinkPreview.astro' // [!code --]
  import { LinkPreview } from '@yohn-maistre/astro-pure-fork/advanced' // [!code ++]

  <LinkPreview href='https://www.cloudflare.com/' />
  ```

- Move `Substats` config to related page `src/pages/about/index.astro`.
- Changed Github Card component to `GithubCard` in `src/components/advanced`. See [Advanced Components #Github Card](http://localhost:4321/mind-garden/integrations/advanced#github-card) for usage.
- Move Links Logbook to `src/site.config.ts`. See [Friend Links #Basic Configurations](/mind-garden/integrations/links#basic-configurations).

### v3.1.3

- Move user components to `src/components/user`. Affected components are included in [User Components](/mind-garden/integrations/components). Import method is also changed:
...
- Replace `remark-github-blockquote-alert` plugin to component `Aside`. Check [User Components #Aside](/mind-garden/integrations/components#aside) to know how to migrate.
