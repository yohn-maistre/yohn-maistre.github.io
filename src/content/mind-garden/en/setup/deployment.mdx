...
import { Aside, Steps } from '@yohn-maistre/astro-pure-fork/user'
...
This method can also work on Deployment for other platforms like 'Vercel'. All you need is to add the environment variable to build options on your platform control settings. The build command `@yohn-maistre/astro-pure-fork check && astro check && astro build` will automatically link the package.
...
For ones deploying on GitHub Pages with a base path, like `username.github.io/repo-name`, please read [Deployment#Platform with Base Path](/mind-garden/setup/deployment#platform-with-base-path).
carefully.
...
2. Update all internal links in Dynamic script files:
   ...
   - `src/pages/mind-garden/DocsContent.astro`
   - `src/pages/rss.xml.ts`
   - `packages/pure/components/pages/PostPreview.astro` ([Customize Theme](/mind-garden/advanced/customize) is required to make the change)
